      ...
      const mockReviews: Review[] = [
        {
          ...
          role_context: "landlord",
          rating: 5,
          content: "Excellent experience renting from this landlord. Extremely responsive to maintenance requests and very professional throughout the entire lease.",
          created_at: new Date(Date.now() - 1000 * 60 * 60 * 24 * 14).toISOString(), // 14 days ago
        },
        {
          id: "rev-2",
          target_user_id: targetUserId,
          reviewer_id: "user-xyz",
          reviewer_name: "Michael Chen",
          reviewer_avatar: null,
          role_context: "contractor",
          rating: 4,
          content: "Did some great renovation work on our event space. Was a bit delayed due to supply issues, but the final result was fantastic and reasonably priced.",
        ...
        <div className="lg:col-span-2 space-y-6">
          {loading ? (
            <div className="space-y-6">
              {[1, 2].map(i => (
                <div key={i} className="animate-pulse bg-slate-50 rounded-2xl p-6 border border-slate-100">
            ...
            <div className="bg-slate-50 rounded-2xl p-10 text-center border border-slate-100 border-dashed">
              ...
              <p className="text-slate-500 mt-1 max-w-sm mx-auto text-sm">Be the first to leave a review and share your experience with others.</p>
            </div>
          ) : (
            <ul className="space-y-6">
              {reviews.map((review) => {
                ...
                return (
                  <li key={review.id} className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm hover:shadow-md transition-shadow duration-200">
                    <div className="flex items-start justify-between gap-4 mb-4">
                      ...
                      <div className="text-right flex flex-col items-end">
                        ...
                        <span className={`mt-1.5 inline-block text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded-full ${
                          isContractor 
          ...
          <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-lg shadow-slate-200/40 sticky top-24">
            ...
            {!user ? (
              <div className="bg-slate-50 rounded-xl p-5 border border-slate-100 text-center">
                <p className="text-slate-600 text-sm mb-4">You must be signed in to share your experience.</p>
                <Link href={`/signin?redirect=/user/${targetUserId}`} className="inline-flex items-center justify-center w-full px-4 py-2.5 bg-slate-900 text-white text-sm font-medium rounded-lg hover:bg-slate-800 transition-colors">
                  Sign In
                </Link>
              </div>
